...
<<recap_sel,echo=FALSE,warning=FALSE>>=
...
<<graphe_p_sel,echo=FALSE,fig.height=6>>=
...
<<compl_sel, echo=FALSE, warning=FALSE,fig.width=8>>= 
ch <- d1[d1$FINESS==hop,]
x <- completude(d1)
y <- completude(ch)
radar_completude(x,y, ch.names = "CH Sélestat")
@
