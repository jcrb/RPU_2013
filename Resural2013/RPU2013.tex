...
soit une moyenne de \np{945} passages par jour (extrêmes 675 et \np{1 180})
...
\caption{Passages en SU en 2013.}
...
\caption{SU d'Alsace: nombre de passages en 2013.}
...
\caption{2013 - Nombre de RPU par mois.}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: l'argument 2 correspond à plusieurs arguments formels}}\end{kframe}% latex table generated in R 3.0.3 by xtable 1.7-3 package
% Sun Mar 30 19:41:42 2014
\begin{table}[ht]
\centering
\begin{tabular}{rr}
  \hline
...
\caption{Activité hebdomadaire en 2013.}
\label{fig:act_sem}
\end{center}
%\end{figure}
...
% table(m)
% barplot(table(m),names.arg=c("Dim","Lun","Mar","Mer","Jeu","Ven","Sam"), main="Activité selon le jour de la semaine")
% @
% 
% %------------------------------
...
% \begin{itemize}
%   \item admission diurne: admission_diurne \%
%   \item Recours nocturne: 100-admission_diurne \%
%   \item sortie diurne:    sortie_diurne \%
%   \item ratio entrée/sortie diurne: round(e_diurne/s_diurne, 2)
%   \item ratio entrée/sortie nocturne: round(e_nocturne/s_nocturne, 2)
% \end{itemize}
% \end{boxedminipage}
% 
% %-----------------------------------------
...
% \begin{boxedminipage}{10cm}
% \begin{itemize}
%   \item entrées en semaine  n_se
%   \item entrées le weekend:  n_we
%   \item pourcentage des entrées en semaine:  round(n_se*100/(n_se+n_we),2) \%
%   \item \textbf{Part d'activité de week-end:   round(n_we*100/(n_se+n_we),2) \%}
%   \item sorties en semaine  n_sse
%   \item sorties le weekend:  n_swe
%   \item pourcentage de sorties en semaine:  round(n_sse*100/(n_sse+n_swe),2)
% \end{itemize}
% \end{boxedminipage}
% 
% %------------------------------------------------
...
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
\includegraphics[width=\maxwidth]{figure/pyr_consult} 
...
\begin{kframe}
\begin{alltt}
\hlstd{d1}\hlopt{$}\hlstd{age3} \hlkwb{<-} \hlkwd{cut}\hlstd{(d1}\hlopt{$}\hlstd{AGE,} \hlkwc{breaks} \hlstd{= brek,} \hlkwc{include.lowest} \hlstd{= F,} \hlkwc{right} \hlstd{= F,} \hlkwc{labels} \hlstd{= x)}
\hlstd{a_table} \hlkwb{<-} \hlkwd{table}\hlstd{(d1}\hlopt{$}\hlstd{SEXE, d1}\hlopt{$}\hlstd{age3)}
\hlcom{# on utilise la transposée de a_table pour que le tableau soit vertical}
\hlcom{# (sinon trop large)}
\hlkwd{xtable}\hlstd{(}\hlkwd{t}\hlstd{(a_table),} \hlkwc{caption} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"Répartition des consultants aux urgences par tranche de cinq ans en fonction du sexe (I = sexe indéterminé)"}\hlstd{,}
    \hlstr{""}\hlstd{),} \hlkwc{label} \hlstd{=} \hlstr{"tab:sr2"}\hlstd{)}
\end{alltt}
\end{kframe}% latex table generated in R 3.0.3 by xtable 1.7-3 package
% Sun Mar 30 19:41:49 2014
\begin{table}[ht]
\centering
\begin{tabular}{rrrr}
  \hline
...
% Date and time: dim., mars 30, 2014 - 19:42:14
...
% Table created by stargazer v.5.0 by Marek Hlavac, Harvard University. E-mail: hlavac at fas.harvard.edu
% Date and time: dim., mars 30, 2014 - 19:42:15
\begin{table}[!htbp] \centering 
