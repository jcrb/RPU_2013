...
h<-hour(AVC$ENTREE)
hist(h,breaks=24,xlab="Heure de la journée",main="Répartition des AVC dans la journée", ylab="Fréquence", col="azure")
@
...
tab1(ait,missing=FALSE, main="Distribution des AIT", ylab="Fréquence")
@
...
Les pneumonies bactériennes concernent les adultes âgés des deux sexes. L'âge moyen est de \Sexpr{p[4]} ans et la moitié de ces patients ont \Sexpr{p[3]} ans et plus (table \ref{tab:pneumo_age} pp.\pageref{tab:pneumo_age}).
...
x<-barplot(table(m),main="Gastro-entérites - 2013",xlab="Mois")
