...
<<passage_clean,echo=FALSE,comment="">>=
...
<<passages_erronés, echo=FALSE, comment="">>= 
...
<<hist_tous_passages,echo=FALSE,fig.width=8>>=
  ...
  a <- a[!is.na(a)]
  a<-a[a > 10 & a < 1440]
  hist(a, breaks=30, xlab="temps de passage (mn)",main="Histogramme du temps de passage",ylab="nombre")
@
 \caption{Histogramme des passages en 2013 (\Sexpr{length(a)} patients). Ne sont pris en compte que les RPU dont la durée de passage est renseignée et inférieure à 24 heures.}
...
xtable(x, caption=c("Répartition des RPU par tranches d'age ","Répartition des RPU par tranches d'age"), label="tab:tranches_age")
@
...
t <- t(t)
rownames(t)<-"mn"
xtable(as.data.frame(t(t)), caption=c("Durée de passage (mn) en fonction de l'âge","Durée de passage et age"), label="tab:age_dp")
@
