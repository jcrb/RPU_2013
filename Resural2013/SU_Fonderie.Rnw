...
<<recap_dia,echo=FALSE,warning=FALSE>>=
...
\hline 
Age moyen & \Sexpr{a["age_moyen"]} ans $\pm \Sexpr{a["age_sd"]}$ \tabularnewline
\hline 
...
<<graphe_fonderie,echo=FALSE,fig.height=6>>=
...
<<compl_dia, echo=FALSE, warning=FALSE,fig.width=8>>= 
ch <- d1[d1$FINESS==hop,]
x <- completude(d1)
y <- completude(ch)
radar_completude(x,y, ch.names = "Diaconnat-Fonderie")
@
