...
<<recap_alk,echo=FALSE,warning=FALSE>>=
...
<<graphe_p_alk,echo=FALSE,fig.height=6>>=
...
<<compl_alk, echo=FALSE, warning=FALSE,fig.width=8>>= 
ch <- d1[d1$FINESS==hop,]
x <- completude(d1)
y <- completude(ch)
radar_completude(x,y, ch.names = "CH d'Alkirch")
@